...
def installPassport():
    ...
    passport_url = 'https://ox.gluu.org/npm/passport/passport-master.tgz'
    passport_modules_url = 'https://ox.gluu.org/npm/passport/passport-master-node_modules.tar.gz'
    ...
    # wget -nv --no-check-certificate https://ox.gluu.org/npm/passport/passport-master.tgz
    if not os.path.exists(passport_fn):
        print "Downloading {}".format(passport_fn)
        setupObj.run(['wget', '-nv', '--no-check-certificate', passport_url, '-O', passport_fn])
