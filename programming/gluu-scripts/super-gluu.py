...
try:
    ...
    print
    "Super-Gluu. Load. Failed to load licensing API"
...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, customScript, configurationAttributes):
        print
        "Super-Gluu. Initialization"

        if not configurationAttributes.containsKey("authentication_mode"):
            print
            "Super-Gluu. Initialization. Property authentication_mode is mandatory"
            return False
        ...
        if StringHelper.isEmpty(authentication_mode):
            print
            "Super-Gluu. Initialization. Failed to determine authentication_mode. authentication_mode configuration parameter is empty"
            return False
        ...
        if not (self.oneStep or self.twoStep):
            print
            "Super-Gluu. Initialization. Valid authentication_mode values are one_step and two_step"
            return False
        ...
        if configurationAttributes.containsKey("super_gluu_group"):
            ...
            print
            "Super-Gluu. Initialization. Using super_gluu only if user belong to group: %s" % self.super_gluu_group
        ...
        if configurationAttributes.containsKey("audit_group"):
            ...
            if (not configurationAttributes.containsKey("audit_group_email")):
                print
                "Super-Gluu. Initialization. Property audit_group_email is not specified"
                return False
            ...
            print
            "Super-Gluu. Initialization. Using audit group: %s" % self.audit_group

        if self.use_super_gluu_group or self.use_audit_group:
            if not configurationAttributes.containsKey("audit_attribute"):
                print
                "Super-Gluu. Initialization. Property audit_attribute is not specified"
                return False
            else:
        ...
        if has_license_api and configurationAttributes.containsKey("license_file"):
            ...
            try:
                ...
                print
                "Super-Gluu. Initialization. Failed to load license from file: %s" % license_file
                return False
            finally:
                f.close()
            ...
            # Validate license
            try:
                self.license_content = LicenseValidator.validate(license["public_key"], license["public_password"],
                                                                 license["license_password"], license["license"],
                                                                 Product.SUPER_GLUU, Date())
                self.valid_license = self.license_content.isValid()
            except:
                print
                "Super-Gluu. Initialization. Failed to validate license. Exception: ", sys.exc_info()[1]
                return False

            print
            "Super-Gluu. Initialization. License status: '%s'. License metadata: '%s'" % (
            self.valid_license, self.license_content.getMetadata())

        print
        "Super-Gluu. Initialized successfully. oneStep: '%s', twoStep: '%s', pushNotifications: '%s', customLabel: '%s'" % (
    ...
    def destroy(self, configurationAttributes):
        ...
        print
        "Super-Gluu. Destroyed successfully"
        return True
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        ...
        if client_redirect_uri == None:
            print
            "Super-Gluu. Authenticate. redirect_uri is not set"
            return False
        ...
        if StringHelper.isNotEmpty(form_auth_result):
            print
            "Super-Gluu. Authenticate for step %s. Get auth_result: '%s'" % (step, form_auth_result)
            ...
            if form_auth_result in ['timeout']:
                if ((step == 1) and self.oneStep) or ((step == 2) and self.twoStep):
                    print
                    "Super-Gluu. Authenticate for step %s. Reinitializing current step" % step
        ...
        if step == 1:
            print
            "Super-Gluu. Authenticate for step 1"
            ...
            if self.oneStep:
                ...
                if not session_device_status['one_step']:
                    print
                    "Super-Gluu. Authenticate for step 1. u2f_device '%s' is not one step device" % u2f_device_id
                    return False
                ...
                print
                "Super-Gluu. Authenticate for step 1. User '%s' successfully authenticated with u2f_device '%s'" % (
                user_name, u2f_device_id)
                ...
                if (self.use_super_gluu_group):
                    print
                    "Super-Gluu. Authenticate for step 1. Checking if user belong to super_gluu group"
                    ...
                    if (is_member_super_gluu_group):
                        print
                        "Super-Gluu. Authenticate for step 1. User '%s' member of super_gluu group" % authenticated_user.getUserId()
                ...
                if auth_method == 'authenticate':
                    ...
                    u2f_devices_list = deviceRegistrationService.findUserDeviceRegistrations(user_inum,
                                                                                             client_redirect_uri,
                                                                                             "oxId")
                    if u2f_devices_list.size() == 0:
                        auth_method = 'enroll'
                        print
                        "Super-Gluu. Authenticate for step 1. There is no U2F '%s' user devices associated with application '%s'. Changing auth_method to '%s'" % (
            ...
            print
            "Super-Gluu. Authenticate for step 2"
            ...
            if self.oneStep and session_device_status['enroll']:
                ...
                if validation_result:
                    print
                    "Super-Gluu. Authenticate for step 2. User '%s' successfully authenticated with u2f_device '%s'" % (
                    user_name, u2f_device_id)
                else:
                    return False
                ...
                print
                "Super-Gluu. Authenticate for step 2. U2F auth_method is invalid"
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if client_redirect_uri == None:
            print
            "Super-Gluu. Prepare for step. redirect_uri is not set"
            return False
        ...
        if step == 1:
            ...
            if self.oneStep:
                ...
                super_gluu_request_dictionary = {'app': client_redirect_uri,
                                                 ...
                                                 'created': DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(
            ...
            if user == None:
                print
                "Super-Gluu. Prepare for step 2. Failed to determine user name"
                return False
            ...
            if session_attributes.containsKey("super_gluu_request"):
                ...
                if not StringHelper.equalsIgnoreCase(super_gluu_request, "timeout"):
                    print
                    "Super-Gluu. Prepare for step 2. Request was generated already"
                    return True
            ...
            if StringHelper.isEmpty(auth_method):
                print
                "Super-Gluu. Prepare for step 2. Failed to determine auth_method"
                return False

            print
            "Super-Gluu. Prepare for step 2. auth_method: '%s'" % auth_method
            ...
            super_gluu_request = json.dumps(super_gluu_request_dictionary, separators=(',', ':'))
            print
            "Super-Gluu. Prepare for step 2. Prepared super_gluu_request:", super_gluu_request
    ...
    def getNextStep(self, configurationAttributes, requestParameters, step):
        ...
        if retry_current_step:
            print
            "Super-Gluu. Get next step. Retrying current step"
    ...
    def validateSessionDeviceStatus(self, client_redirect_uri, session_device_status, user_name=None):
        ...
        if session_device_status['enroll'] and session_device_status['one_step']:
            ...
            if u2f_device == None:
                print
                "Super-Gluu. Validate session device status. There is no one step u2f_device '%s'" % u2f_device_id
                return False
        else:
            ...
            if u2f_device == None:
                print
                "Super-Gluu. Validate session device status. There is no u2f_device '%s' associated with user '%s'" % (
                u2f_device_id, user_inum)
                return False

        if not StringHelper.equalsIgnoreCase(client_redirect_uri, u2f_device.application):
            print
            "Super-Gluu. Validate session device status. u2f_device '%s' associated with other application '%s'" % (
            u2f_device_id, u2f_device.application)
            return False
    ...
    def getSessionDeviceStatus(self, session_attributes, user_name):
        print
        "Super-Gluu. Get session device status"

        if not session_attributes.containsKey("super_gluu_request"):
            print
            "Super-Gluu. Get session device status. There is no Super-Gluu request in session attributes"
            return None

        # Check session state extended
        if not session_attributes.containsKey("session_custom_state"):
            print
            "Super-Gluu. Get session device status. There is no session_custom_state in session attributes"
            return None
        ...
        if not StringHelper.equalsIgnoreCase("approved", session_custom_state):
            print
            "Super-Gluu. Get session device status. User '%s' not approve or not pass U2F authentication. session_custom_state: '%s'" % (
            user_name, session_custom_state)
            return None

        # Try to find device_id in session attribute
        if not session_attributes.containsKey("oxpush2_u2f_device_id"):
            print
            "Super-Gluu. Get session device status. There is no u2f_device associated with this request"
            return None

        # Try to find user_inum in session attribute
        if not session_attributes.containsKey("oxpush2_u2f_device_user_inum"):
            print
            "Super-Gluu. Get session device status. There is no user_inum associated with this request"
            return None
        ...
        session_device_status = {"super_gluu_request": super_gluu_request, "device_id": u2f_device_id,
                                 "user_inum": user_inum, "enroll": enroll, "one_step": one_step}
        print
        "Super-Gluu. Get session device status. session_device_status: '%s'" % (session_device_status)

        return session_device_status

    def initPushNotificationService(self, configurationAttributes):
        print
        "Super-Gluu. Initialize Native/SNS/Gluu notification services"
    ...
    def initNativePushNotificationService(self, configurationAttributes):
        print
        "Super-Gluu. Initialize native notification services"
        ...
        try:
            ...
            print
            "Super-Gluu. Initialize native notification services. Invalid credentials file format"
            return False
        ...
        if android_creds["enabled"]:
            ...
            print
            "Super-Gluu. Initialize native notification services. Created Android notification service"
        ...
        if ios_creds["enabled"]:
            ...
            try:
                ...
                print
                "Super-Gluu. Initialize native notification services. Assuming that 'p12_password' password in not encrypted"
            ...
            print
            "Super-Gluu. Initialize native notification services. Created iOS notification service"
    ...
    def initSnsPushNotificationService(self, configurationAttributes):
        print
        "Super-Gluu. Initialize SNS notification services"
        ...
        try:
            ...
            print
            "Super-Gluu. Initialize SNS notification services. Invalid credentials file format"
            return False
        ...
        if not (android_creds["enabled"] or ios_creds["enabled"]):
            print
            "Super-Gluu. Initialize SNS notification services. SNS disabled for all platforms"
            return False
        ...
        try:
            ...
            # Ignore exception. Password is not encrypted
            print
            "Super-Gluu. Initialize SNS notification services. Assuming that 'sns_secret_access_key' in not encrypted"
        ...
        if android_creds["enabled"]:
            ...
            print
            "Super-Gluu. Initialize SNS notification services. Created Android notification service"
        ...
        if ios_creds["enabled"]:
            ...
            self.pushAppleServiceProduction = ios_creds["production"]
            print
            "Super-Gluu. Initialize SNS notification services. Created iOS notification service"
    ...
    def initGluuPushNotificationService(self, configurationAttributes):
        print
        "Super-Gluu. Initialize Gluu notification services"
        ...
        try:
            ...
            print
            "Super-Gluu. Initialize Gluu notification services. Invalid credentials file format"
            return False
        ...
        if not (android_creds["enabled"] or ios_creds["enabled"]):
            print
            "Super-Gluu. Initialize Gluu notification services. Gluu disabled for all platforms"
            return False
        ...
        try:
            metadataConfiguration = notifyClientFactory.createMetaDataConfigurationService(
                gluu_server_uri).getMetadataConfiguration()
        except:
            print
            "Super-Gluu. Initialize Gluu notification services. Failed to load metadata. Exception: ", sys.exc_info()[1]
            return False
        ...
        if android_creds["enabled"]:
            ...
            try:
                ...
                # Ignore exception. Password is not encrypted
                print
                "Super-Gluu. Initialize Gluu notification services. Assuming that 'gluu_secret_access_key' in not encrypted"
            ...
            print
            "Super-Gluu. Initialize Gluu notification services. Created Android notification service"
        ...
        if ios_creds["enabled"]:
            ...
            print
            "Super-Gluu. Initialize Gluu notification services. Created iOS notification service"
    ...
    def loadPushNotificationCreds(self, configurationAttributes):
        print
        "Super-Gluu. Initialize notification services"
        ...
        try:
            ...
            print
            "Super-Gluu. Initialize notification services. Failed to load credentials from file:", super_gluu_creds_file
            return None
        finally:
            f.close()
    ...
    def sendPushNotification(self, client_redirect_uri, user, super_gluu_request):
        try:
            ...
            print
            "Super-Gluu. Send push notification. Failed to send push notification: ", sys.exc_info()[1]
    ...
    def sendPushNotificationImpl(self, client_redirect_uri, user, super_gluu_request):
        ...
        user_name = user.getUserId()
        print
        "Super-Gluu. Send push notification. Loading user '%s' devices" % user_name
            ...
            for u2f_device in u2f_devices_list:
                ...
                if StringHelper.equalsIgnoreCase(platform, "ios") and StringHelper.isNotEmpty(push_token):
                    ...
                    if self.pushAppleService == None:
                        print
                        "Super-Gluu. Send push notification. Apple native push notification service is not enabled"
                    else:
                        ...
                        if self.pushSnsMode or self.pushGluuMode:
                            pushSnsService = CdiUtil.bean(PushSnsService)
                            targetEndpointArn = self.getTargetEndpointArn(deviceRegistrationService, pushSnsService,
                            ...
                            if self.pushSnsMode:
                                ...
                                send_notification_result = pushSnsService.sendPushMessage(self.pushAppleService,
                                                                                          apple_push_platform,
                                                                                          targetEndpointArn,
                                                                                          push_message, None)
                                if debug:
                                    print
                                    "Super-Gluu. Send iOS SNS push notification. token: '%s', message: '%s', send_notification_result: '%s', apple_push_platform: '%s'" % (
                                    push_token, push_message, send_notification_result, apple_push_platform)
                            elif self.pushGluuMode:
                                send_notification_result = self.pushAppleService.sendNotification(
                                    self.pushAppleServiceAuth, targetEndpointArn, push_message)
                                if debug:
                                    print
                                    "Super-Gluu. Send iOS Gluu push notification. token: '%s', message: '%s', send_notification_result: '%s'" % (
                                    push_token, push_message, send_notification_result)
                        else:
                            ...
                            if debug:
                                print
                                "Super-Gluu. Send iOS Native push notification. token: '%s', message: '%s', send_notification_result: '%s'" % (
                                push_token, push_message, send_notification_result)
                        send_ios = send_ios + 1
                ...
                if StringHelper.equalsIgnoreCase(platform, "android") and StringHelper.isNotEmpty(push_token):
                    ...
                    if self.pushAndroidService == None:
                        print
                        "Super-Gluu. Send native push notification. Android native push notification service is not enabled"
                    else:
                        send_notification = True
                        ...
                        if self.pushSnsMode or self.pushGluuMode:
                            ...
                            if self.pushSnsMode:
                                ...
                                if debug:
                                    print
                                    "Super-Gluu. Send Android SNS push notification. token: '%s', message: '%s', send_notification_result: '%s'" % (
                                    push_token, push_message, send_notification_result)
                            elif self.pushGluuMode:
                                send_notification_result = self.pushAndroidService.sendNotification(
                                    self.pushAndroidServiceAuth, targetEndpointArn, push_message)
                                if debug:
                                    print
                                    "Super-Gluu. Send Android Gluu push notification. token: '%s', message: '%s', send_notification_result: '%s'" % (
                                    push_token, push_message, send_notification_result)
                        else:
                            msgBuilder = Message.Builder().addData("message", super_gluu_request).addData("title",
                                                                                                          title).collapseKey(
                                "single").contentAvailable(True)
                            push_message = msgBuilder.build()

                            send_notification_result = self.pushAndroidService.send(push_message, push_token, 3)
                            if debug:
                                print
                                "Super-Gluu. Send Android Native push notification. token: '%s', message: '%s', send_notification_result: '%s'" % (
                                push_token, push_message, send_notification_result)
                        send_android = send_android + 1

        print
        "Super-Gluu. Send push notification. send_android: '%s', send_ios: '%s'" % (send_android, send_ios)
    ...
    def getTargetEndpointArn(self, deviceRegistrationService, pushSnsService, platform, user, u2fDevice):
        ...
        if StringHelper.isNotEmpty(notificationConf):
            ...
            if StringHelper.isNotEmpty(targetEndpointArn):
                print
                "Super-Gluu. Get target endpoint ARN. There is already created target endpoint ARN"
        ...
        print
        "Super-Gluu. Get target endpoint ARN. Attempting to create target endpoint ARN for user: '%s'" % user.getUserId()
        if self.pushSnsMode:
        ...
        if StringHelper.isEmpty(targetEndpointArn):
            print
            "Super-Gluu. Failed to get endpoint ARN for user: '%s'" % user.getUserId()
            return None

        print
        "Super-Gluu. Get target endpoint ARN. Create target endpoint ARN '%s' for user: '%s'" % (
    ...
    def addGeolocationData(self, session_attributes, super_gluu_request_dictionary):
        if session_attributes.containsKey("remote_ip"):
            ...
            if StringHelper.isNotEmpty(remote_ip):
                print
                "Super-Gluu. Prepare for step 2. Adding req_ip and req_loc to super_gluu_request"
                ...
                if remote_loc_dic == None:
                    print
                    "Super-Gluu. Prepare for step 2. Failed to determine remote location by remote IP '%s'" % remote_ip
                    return

                remote_loc = "%s, %s, %s" % (
                remote_loc_dic['country'], remote_loc_dic['regionName'], remote_loc_dic['city'])
    ...
    def determineGeolocationData(self, remote_ip):
        print
        "Super-Gluu. Determine remote location. remote_ip: '%s'" % remote_ip
        ...
        try:
            ...
            print
            "Super-Gluu. Determine remote location. Exception: ", sys.exc_info()[1]
            return None

        try:
            if not httpService.isResponseStastusCodeOk(http_response):
                print
                "Super-Gluu. Determine remote location. Get invalid response from validation server: ", str(
        ...
        if response_string == None:
            print
            "Super-Gluu. Determine remote location. Get empty response from location server"
            return None
        ...
        if not StringHelper.equalsIgnoreCase(response['status'], "success"):
            print
            "Super-Gluu. Determine remote location. Get response with status: '%s'" % response['status']
            return None
    ...
    def processAuditGroup(self, user, attribute, group):
        ...
        if (is_member):
            print
            "Super-Gluu. Authenticate for processAuditGroup. User '%s' member of audit group" % user.getUserId()
            print
